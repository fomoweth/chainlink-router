...
contract FeedConfigTest is Test {
	function test_fuzz_toFeedConfig(
		address feed,
		uint8 feedDecimals,
		uint16 baseId,
		uint8 baseDecimals,
		...
		FeedConfig config = toFeedConfig(feed, feedDecimals, baseId, baseDecimals, quoteId, quoteDecimals);
		assertEq(config.feed(), feed);
		assertEq(config.feedDecimals(), feedDecimals);

...
contract ChainlinkRouterTest is Test, BaseTest {
    ...
    function test_initialize() public view {
        ...
        assertEq(router.getAssetId(Denominations.USD), 0);
        assertEq(router.getAsset(0), Denominations.USD);
        ...
        for (uint256 i; i < feeds.length; ++i) {
            ...
            assertEq(router.getFeed(assets[i], Denominations.USD), feeds[i]);
        }
    }
    ...
    function test_deregisterAsset_revertsOnInvalidAsset() public {
        ...
        vm.expectRevert(IChainlinkRouter.InvalidAsset.selector);
        router.deregisterAsset(Denominations.USD);
    }
    ...
    function test_register() public {
        ...
        emit IChainlinkRouter.FeedRegistered(LINK_USD, LINK, Denominations.USD);

        router.register(abi.encodePacked(LINK_USD, LINK, Denominations.USD));

        assertEq(router.getFeed(LINK, Denominations.USD), LINK_USD);
        assertEq(router.numAssets(), initialCount + 1);
    }
    ...
    function test_register_all() public {
        ...
        uint256[] memory chainIds =
            SolArray.uint256s(Chains.ETHEREUM, Chains.OPTIMISM, Chains.POLYGON, Chains.BASE, Chains.ARBITRUM);

        uint256[] memory blockNumbers = SolArray.uint256s(
            ETHEREUM_FORK_BLOCK, OPTIMISM_FORK_BLOCK, POLYGON_FORK_BLOCK, BASE_FORK_BLOCK, ARBITRUM_FORK_BLOCK
        );
        ...
        for (uint256 i; i < chainIds.length; ++i) {
            ...
            bytes memory params = runScript("encode", vm.toString(chainIds[i]));
            router.register(params);
    ...
    function test_register_revertsIfUnauthorized() public impersonate(unknown) {
        vm.expectRevert(abi.encodeWithSelector(Ownable.UnauthorizedAccount.selector, unknown));
        router.register(abi.encodePacked(LINK_ETH, LINK, WETH, LINK_USD, LINK, Denominations.USD));
    }
    ...
    function test_deregister() public {
        ...
        emit IChainlinkRouter.FeedDeregistered(LINK, Denominations.USD);

        router.deregister(abi.encodePacked(LINK, Denominations.USD));

        assertEq(router.getAssetId(LINK), 0);
        assertEq(router.getAsset(assetId), address(0));
        assertEq(router.getFeed(LINK, Denominations.USD), address(0));
        assertEq(router.numAssets(), initialCount - 1);
    }
    ...
    function test_getAssetConfiguration() public view {
        BitMap configuration = router.getAssetConfiguration(Denominations.USD);
    ...
    function test_getFeedConfiguration() public view {
        ...
        for (uint256 i; i < feeds.length; ++i) {
            FeedConfig configuration = router.getFeedConfiguration(assets[i], Denominations.USD);
            assertFalse(configuration.isZero());
            ...
            assertEq(configuration.quoteId(), router.getAssetId(Denominations.USD));
            assertEq(configuration.quoteDecimals(), 8);
            assertTrue(router.getAssetConfiguration(assets[i]).get(router.getAssetId(Denominations.USD)));
        }
    }
    ...
    function test_queryFeed_findsDirectFeed() public view {
        assertEq(router.queryFeed(WETH, Denominations.USD), ETH_USD);
    }

    function test_queryFeed_findsBidirectionalFeed() public view {
        assertEq(router.queryFeed(Denominations.USD, WETH), ETH_USD);
    }
    ...
    function test_getFeed_exactDirectionOnly() public view {
        assertEq(router.getFeed(WETH, Denominations.USD), ETH_USD);
        assertEq(router.getFeed(Denominations.USD, WETH), address(0));
    }
    ...
    function test_query_singleHop() public view {
        ...
        for (uint256 i; i < feeds.length; ++i) {
            (address[] memory path, uint256 price) = router.query(assets[i], Denominations.USD);
    ...
    function test_query_singleHop_inverted() public view {
        ...
        for (uint256 i; i < feeds.length; ++i) {
            (address[] memory path, uint256 price) = router.query(Denominations.USD, assets[i]);
            uint256 expected = getInverseAnswer(feeds[i], assets[i], Denominations.USD);
    ...
    function test_query_3Hops() public {
        ...
        bytes memory params =
            abi.encodePacked(LINK_ETH, LINK, WETH, AAVE_ETH, AAVE, WETH, COMP_ETH, COMP, WETH, UNI_ETH, UNI, WETH);
    ...
    function test_query_3Hops_inverted() public {
        ...
        for (uint256 i; i < ethFeeds.length; ++i) {
            ...
            uint256 expected = getInverseAnswer(usdFeeds[i], assets[i], Denominations.USD);
    ...
    function test_query_revertsOnNegativePrice() public {
        ...
        router.query(WETH, Denominations.USD);
    }
